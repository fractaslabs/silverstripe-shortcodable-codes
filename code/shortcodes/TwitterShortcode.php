...
class TwitterShortcode extends ViewableData
{
    ...
    public static function parse_shortcode($arguments, $content, $parser, $shortcode)
    {
        if (!array_key_exists('link', $arguments) || !$arguments['link']) {
            return 'Twitter Shortcode: no link';
        }

        $arguments['link'] = str_replace("'", '', $arguments['link']);
        if (strpos($arguments['link'], ',')) {
            return 'Twitter Shortcode: only one link supported at the moment';
        }

        $apiUrl = self::API_URL;
        $query = array('url' => Convert::raw2xml($arguments['link']));
        if (array_key_exists('isvideo', $arguments)) {
            $query['widget_type'] = 'video';
            $apiUrl = self::API_VIDEO_URL;
        }
        ...
        return $data->renderWith('TwitterShortcode');
    }
